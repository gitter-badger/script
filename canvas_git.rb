...
# at commit A to point at another commit B, it is called a fast-forward update if
# and only if B is a descendant of A.

# In a fast-forward update from A to B, the set of commits that the original commit A
# built on top of is a subset of the commits the new commit B builds on top of.
...
# leading to commit B while the other person built a history leading to commit A.
...
# to the original repository you two obtained the original commit X.
...
# with commit B. This does not fast-forward. If you did so, the changes introduced
# by commit A will be lost, because everybody will now start building on top of B.
...
# other person (history from X to A), you would need to first fetch the history from
# the repository, create a history that
...
# commit D that builds the change between X and B
# on top of A.
...
# to produce commit B, and you try to push it out, because forgot that you have pushed A out
# already. In such a case, and only if you are certain that nobody in the meantime
